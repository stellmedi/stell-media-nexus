...
const ClientLogos = () => {
  ...
  return <section className="py-12 bg-gray-50" aria-label="Our trusted clients">
      <div className="container mx-auto px-4">
        <div className="text-center mb-8">
          <h2 className="text-2xl font-semibold text-gray-900 mb-2">Trusted by Leading E-commerce Brands</h2>
          <p className="text-gray-600">Join 50+ companies that trust Stell Media with their product discovery optimization</p>
        </div>
        
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 items-center">
          {clients.map((client, index) => <div key={index} className="flex items-center justify-center p-4 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300 grayscale hover:grayscale-0">
              <img src={client.logo} alt={client.alt} className="max-h-12 w-auto object-contain opacity-60 hover:opacity-100 transition-opacity duration-300" loading="lazy" />
            </div>)}
        </div>
        ...
        <div className="text-center mt-8">
          <div className="inline-flex items-center gap-2 text-sm text-gray-500">
            ...
            <span>Backed by 15+ years of experience </span>
          </div>
        </div>
      </div>
    </section>;
