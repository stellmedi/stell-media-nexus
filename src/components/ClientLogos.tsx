...
const ClientLogos = () => {
  ...
  return (
    <section className="py-16 bg-gray-50" aria-label="Our trusted clients">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8 items-center justify-items-center">
          {clients.map((client, index) => (
            <div 
              key={index} 
              className="group flex items-center justify-center p-6 bg-white rounded-xl shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-100 hover:border-indigo-200 w-full h-24"
            >
              <img 
                src={client.logo} 
                alt={client.alt} 
                className="max-h-12 max-w-full w-auto object-contain opacity-70 group-hover:opacity-100 transition-opacity duration-300 filter grayscale group-hover:grayscale-0" 
                loading="lazy"
                width="160"
                height="48"
              />
